module.exports = function(name, redisOpts) {
  ...
  var _redisPort = redisOpts.port || 6379;
  var _redisHost = redisOpts.host || 'verhogen.com';
  var _redisPass = redisOpts.password;

  var _holdingLock = false;
  var _redis       = require('redis');
  var _client      = _redis.createClient(_redisPort, _redisHost);
  _client.on('error', function(err) {
    console.log('Error ' + err);
  });
  _client.auth(_redisPass, function(err, reply) {
    if (err) {
